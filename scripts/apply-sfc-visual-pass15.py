...
.board[data-preview="true"]{box-shadow:0 0 0 2px #08070a,0 0 0 5px #fff0a0,0 6px 0 #000}.board[data-preview="true"]::after{opacity:1}
...
if 'SFC visual reconstruction pass 15 — prism board housing' not in css:
    ...
    css = css.replace('.board:has(.preview){', '.board[data-preview="true"]{').replace('.board:has(.preview)::after{', '.board[data-preview="true"]::after{')
css_path.write_text(css.rstrip() + '\n')

tsx_path = Path('app/rpg/RPGPuzzleBattle.tsx')
tsx = tsx_path.read_text()
old = '<section className={styles.board} aria-label="RPG Cluster Break board">'
new = '<section className={styles.board} data-preview={preview ? "true" : "false"} aria-label="RPG Cluster Break board">'
if old in tsx:
