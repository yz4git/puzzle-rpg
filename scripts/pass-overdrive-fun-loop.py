...
rep('  function addBank(points: number, comboValue: number, chainDepth = 0) {','''  function addMomentum(amount = 1) {
    ...
    momentumRef.current = next; setMomentum(next);
    if (next >= MOMENTUM_GOAL) { surgeReadyRef.current = true; setSurgeReady(true); setLastRank("PRISM SURGE READY!"); setMessage("NEXT BIG BREAK GETS ×2 • BUILD ×5+"); playOverdriveSfx("target", 1.16); }
  }
  function consumeSurge(count: number) {
    if (!surgeReadyRef.current || count < 5) return 1;
    surgeReadyRef.current = false; setSurgeReady(false); momentumRef.current = 0; setMomentum(0);
    setLastRank("PRISM SURGE ×2!"); setMessage("SURGE RELEASE • BIG BREAK DOUBLED"); playOverdriveSfx("mega", 1.38); return 2;
  }
