...
const outDir = 'review-output/early-battle-20260828';
...
const initialSave = {
  ...
  mapId: 'oldTemple', position: { x: 8, y: 11 }, direction: 'down',
...
// Force only the encounter selection to choose the last Old Temple table entry: HOLLOW MONK.
await page.evaluate(() => { window.__reviewRandom = Math.random; Math.random = () => 0.999; });
await page.getByRole('button', { name: 'Move down', exact: true }).tap();
await page.waitForFunction(() => document.querySelector('main[data-encounter]')?.getAttribute('data-encounter') !== 'none');
await page.evaluate(() => { if (window.__reviewRandom) Math.random = window.__reviewRandom; });
await page.waitForSelector('main[data-enemy="null"]', { timeout: 2500 });
...
function parse(text) {
  ...
  return {
    ...
    nowLabel: now?.[1] || '', nowPower: Number(now?.[2] || 0), nowDetail: now?.[3] || ''
  };
}
...
async function waitResolution(prevTurn) {
  ...
  await page.waitForFunction(({ prevTurn }) => {
    ...
    return Number(match?.[1] || 0) > prevTurn || !!document.querySelector('[class*=resultOverlay]');
  }, { prevTurn }, { timeout: 2400 }).catch(() => {});
  await page.waitForTimeout(180);
}
...
for (let i = 0; i < 12; i += 1) {
  ...
  const beforeText = await battle.innerText();
  const before = parse(beforeText);
  const allGroups = groups(await cells());
  const candidates = Object.fromEntries(['ATK','HEAL','BAR','SKIP'].map(type => [type, allGroups.filter(g => g.type === type).sort((a,b)=>b.count-a.count)[0] || null]));
  let chosen = candidates.ATK || allGroups[0];
  // Simple human-like early-game policy: block strong non-piercing hits, heal low HP, otherwise attack; use SKIP only when it is a strong cluster.
  ...
  const afterText = still ? await battle.innerText() : await page.locator('body').innerText();
  const after = still ? parse(afterText) : null;
  records.push({ before, largest: Object.fromEntries(Object.entries(candidates).map(([k,v]) => [k, v?.count || 0])), chosen: { type: chosen.type, count: chosen.count, row: seed.row, col: seed.col }, elapsedMs: Date.now()-started, after, stillBattle: still });
...
console.log('HOLLOW MONK BALANCE REVIEW', JSON.stringify({ turnsPlayed: records.length, errors: errors.length, last: records.at(-1), finalSave: finalSave && { hp: finalSave.hp, level: finalSave.level, exp: finalSave.exp, gold: finalSave.gold } }));
await browser.close();
