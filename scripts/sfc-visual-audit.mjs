...
const out = 'sfc-visual-audit';
await mkdir(out, { recursive: true });
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
const browser = spawn('google-chrome', [
  '--headless=new', '--no-sandbox', '--disable-gpu', '--hide-scrollbars',
  '--remote-debugging-port=9222', '--user-data-dir=/tmp/sfc-visual-audit',
...
async function metrics(cdp) {
  return ev(cdp, `(()=>{
    ...
    return {
      ...
      world:rect('canvas[aria-label$="exploration map"]'),
      board:rect('[aria-label="RPG Cluster Break board"]'),
      talk:rect('[class*="talkMoment"]'),
      menu:rect('[class*="menuWindow"]'),
      enemy:rect('[role="img"]')
    };
  })()`);
}
...
try {
  ...
  await shot(cdp,'01-title','Mode select');
  ...
  await sleep(250);
  await shot(cdp,'02-hearth-village','First controllable village; larger characters and denser frame');

  await key(cdp,'b',160); await shot(cdp,'03-field-menu','Compact SNES-style field menu');
  await clickText(cdp,'B • CLOSE'); await sleep(120);

  // Start position (8,10): step down to face portal at (8,12), then A.
  await key(cdp,'ArrowDown',90); await key(cdp,'a',280);
  await shot(cdp,'04-prism-road','Prism Road field presentation');

  // Leave road and generate a natural encounter.
  ...
  await sleep(250); await shot(cdp,'05-battle','Scene backdrop + enlarged puzzle board');
  const board = (await metrics(cdp)).board;
  if (!board || board.w < 340) throw new Error(`RPG battle board too small: ${board?.w}`);

  await clickText(cdp,'RPG COMMAND'); await sleep(120); await shot(cdp,'06-command','RPG command overlay');
  await clickText(cdp,'TALK'); await sleep(180); await shot(cdp,'07-talk-moment','Dedicated TALK reaction window before enemy action');
  await sleep(1000); await shot(cdp,'08-after-talk','Battle after TALK resolves');

  // Reload to title, then verify Chapter Battle is unaffected by RPG visual changes.
  ...
  await sleep(200); await shot(cdp,'09-chapter-battle','Existing Chapter Battle regression check');
