...
const browser = spawn(chrome, [
  ...
  '--remote-debugging-port=9222', '--user-data-dir=/tmp/puzzle-rpg-chapter1-audit',
...
function scoreGroup(group, state) {
  ...
  if (group.type === 'ATK') return count * 3.2 + (count >= state.enemyHp ? 100 : 0) + (count >= 6 ? 8 : 0);
  if (group.type === 'SKIP') return count >= 2 ? count * 4 + 5 : 1;
  if (group.type === 'HEAL') {
    const effective = Math.min(count, Math.max(0, 20 - state.hp));
    return effective * 4.5 + (state.hp <= 10 ? 16 : 0) + count * 0.15;
  }
  if (group.type === 'BAR') {
    const effective = Math.min(count, Math.max(0, 20 - state.bar));
    const danger = state.free === 0 && state.intentPower > state.bar ? 10 : 0;
    return effective * 2.4 + danger + (count >= 6 ? 4 : 0);
  }
  return count;
}
...
const stateExpr = `(() => {
  ...
  const build = +(top.match(/BUILD\\s+(\\d+)/)?.[1] || 0);
  ...
  for (const tile of tiles) {
    ...
    groups.push({type:tile.type,count:members.length,seedLabel:members[0].label});
  }
  return {
    stage, turn, build, enemyHp, enemyMax, hp, bar, free, intentPower, groups,
    ...
    rewardNames: Array.from(document.querySelectorAll('[aria-label^="Choose reward "]')).map(el => el.getAttribute('aria-label')?.replace('Choose reward ', '') || ''),
    nextStage: Array.from(document.querySelectorAll('button')).some(b => b.textContent.includes('NEXT STAGE')),
    bodyH: document.body.scrollHeight, viewportH: innerHeight,
  };
})()`;
...
try {
  ...
  let action = 0;
  while (action < 45) {
    const state = await evalJs(cdp, stateExpr);
    ...
    if (state.stageClear) {
      ...
      await shot(cdp, `${pad(shotIndex++)}-stage${state.stage}-reward`);
      if (state.stage >= 3) break;
      if (state.rewardNames.length > 0) {
        const preferred = ['BERSERKER','TEMPO BLADE','DEEP FOCUS','WIDE BREAK','OVERHEAL','FORTRESS','TIME THIEF'];
        const chosenName = preferred.find(name => state.rewardNames.includes(name)) || state.rewardNames[0];
        await evalJs(cdp, `Array.from(document.querySelectorAll('[aria-label^="Choose reward "]')).find(b=>b.getAttribute('aria-label')===${JSON.stringify(`Choose reward ${chosenName}`)})?.click()`);
        await sleep(250);
        const acquired = await evalJs(cdp, stateExpr);
        log.push({event:'reward-picked', stage:state.stage, reward:chosenName, state:acquired});
        await shot(cdp, `${pad(shotIndex++)}-stage${state.stage}-acquired`);
      }
    ...
    if (state.stageIntro) {
      await evalJs(cdp, `Array.from(document.querySelectorAll('button')).find(b=>b.textContent.includes('BATTLE START'))?.click()`); await sleep(450); continue;
    }
    if (!state.groups.length) { log.push({event:'no-groups', state}); break; }
    const choice = state.groups.map(g => ({...g, score:scoreGroup(g,state)})).sort((a,b)=>b.score-a.score || b.count-a.count)[0];
    ...
    log.push({event:'action', action, choice:{type:choice.type,count:choice.count}, before, after:{...after,groups:undefined}});
    if (action % 6 === 0) await shot(cdp, `${pad(shotIndex++)}-stage${after.stage}-turn${action}`);
  }
  const finalState = await evalJs(cdp, stateExpr);
  await shot(cdp, `${pad(shotIndex++)}-final`);
  await writeFile(`${outDir}/chapter1-audit.json`, JSON.stringify({ finalState:{...finalState,groups:undefined}, log }, null, 2));
  cdp.ws.close();
} finally {
