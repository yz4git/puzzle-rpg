...
save_path = Path("app/rpg/save.ts")
current = save_path.read_text()
if 'const STORAGE_KEY = "puzzle-rpg:rpg-mode:v1";' not in current:
    raise SystemExit("unexpected save.ts storage key")
if "BACKUP_STORAGE_KEY" in current:
    raise SystemExit("Pass 36 appears to be applied already")

save_path.write_text(r'''import type { Direction, EquipmentId, ItemId, MapDefinition, RPGSaveData, TechniqueId, Vec2 } from "./types";
...
test_path = Path("tests/rpg-save-integrity.test.ts")
...
ci_path = Path(".github/workflows/puzzle-rpg-ci.yml")
ci = ci_path.read_text()
anchor = '''      - name: Install dependencies\n        run: npm ci\n      - name: Puzzle RPG TypeScript check\n'''
replacement = '''      - name: Install dependencies\n        run: npm ci\n      - name: RPG save integrity tests\n        run: node --import tsx --test tests/rpg-save-integrity.test.ts\n      - name: Puzzle RPG TypeScript check\n'''
if anchor not in ci:
    raise SystemExit("CI anchor missing")
ci_path.write_text(ci.replace(anchor, replacement, 1))
