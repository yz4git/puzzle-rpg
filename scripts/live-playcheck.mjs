...
async function snapshot(page, name) {
  ...
  const metrics = await page.evaluate(() => {
    const buttons = [...document.querySelectorAll('button')].filter(el => {
      ...
      return s.visibility !== 'hidden' && s.display !== 'none' && r.width > 0 && r.height > 0 && r.bottom > 0 && r.top < innerHeight && r.right > 0 && r.left < innerWidth;
    }).map(el => {
      const r = el.getBoundingClientRect();
      return {
...
async function tapAnyCluster(page, preferredType = null) {
  const cells = page.locator('button[aria-label*=" row "][aria-label*=" column "]');
  const data = [];
  for (let i = 0; i < await cells.count(); i++) {
    const cell = cells.nth(i);
    const aria = await cell.getAttribute('aria-label');
    const match = aria?.match(/^(ATK|HEAL|BAR|SKIP) row (\d+) column (\d+)$/i);
    if (match) data.push({ locator: cell, type: match[1].toUpperCase(), row: +match[2], col: +match[3] });
  }
  const candidates = preferredType ? data.filter(v => v.type === preferredType) : data;
  for (const a of candidates) {
    const adjacent = data.find(b => b !== a && b.type === a.type && Math.abs(b.row - a.row) + Math.abs(b.col - a.col) === 1);
    if (!adjacent) continue;
    await a.locator.tap({ force: true });
    return true;
  }
  const fallback = candidates[0] ?? data[0];
  if (!fallback) return false;
  await fallback.locator.tap({ force: true });
  return true;
}
...
// 1) Public title + Chapter Battle + one actual board move.
{
  ...
  const moved = await tapAnyCluster(page);
  await page.waitForTimeout(1000);
  const chapterAfter = await snapshot(page, '03-chapter-after-move');
  const turn2 = Number(chapterAfter.text.match(/TURN\s+(\d+)/i)?.[1] ?? 0);
  assert('Chapter Battle accepts a real cluster move', moved && turn2 > turn1, { turn1, turn2 });
...
// 2) Field movement -> random encounter -> STATUS -> cluster -> TALK.
{
  ...
  assert('RPG command exposes TALK ITEM STATUS RUN', ['TALK', 'ITEM', 'STATUS', 'RUN'].every(v => command.text.includes(v)));
  ...
  const cleared = await tapAnyCluster(page);
  await page.waitForTimeout(1200);
  const afterMove = await snapshot(page, '08-rpg-after-cluster');
  const afterTurn = Number(afterMove.text.match(/TURN\s+(\d+)/i)?.[1] ?? 0);
  assert('RPG cluster advances turn', cleared && afterTurn > beforeTurn, { beforeTurn, afterTurn });
  const talked = await talkOnce(page);
  await page.waitForTimeout(1200);
  const afterTalk = await snapshot(page, '09-rpg-after-talk');
  const talkTurn = Number(afterTalk.text.match(/TURN\s+(\d+)/i)?.[1] ?? 0);
  assert('TALK consumes one battle turn', talked && talkTurn > afterTurn, { afterTurn, talkTurn });
  await context.close();
}
...
// 4) HERB -> enemy action -> SMOKE escape -> inventory persistence.
{
  ...
  assert('Used HERB persists after escape', itemSaved.inventory?.find?.(v => v.id === 'herb')?.count === 1, { inventory: itemSaved.inventory });
  assert('Used SMOKE is removed after escape', !itemSaved.inventory?.some?.(v => v.id === 'smoke'), { inventory: itemSaved.inventory });
  await context.close();
}
...
// 5) Defeat -> 15% gold loss -> last inn recovery with progression intact.
{
  ...
  assert('Defeat keeps inventory', recovered.inventory?.find?.(v => v.id === 'herb')?.count === 2, { inventory: recovered.inventory });
  await tap(page, /CONTINUE/i);
...
// 6) First dungeon boss -> victory -> level/reward/flag -> encounter stays cleared.
{
  ...
  let moves = 0;
  while (moves < 40 && await page.locator('main[data-enemy]').count()) {
    const attacked = await tapAnyCluster(page, 'ATK');
    if (!attacked) break;
    moves += 1;
    await page.waitForTimeout(850);
  }
  const bossResult = await snapshot(page, '19-temple-victory');
  assert('Temple boss can be defeated through board actions', /VICTORY/i.test(bossResult.text), { moves });
  assert('Boss victory produces LEVEL UP', /LEVEL UP/i.test(bossResult.text));
  ...
  assert('Boss victory returns to OLD TEMPLE', /OLD TEMPLE/i.test(templeReturn.text));
  await tap(page, /A\s*CHECK/i);
  await page.waitForTimeout(500);
  assert('Cleared boss does not reopen when checked again', await page.locator('main[data-enemy]').count() === 0);
  await context.close();
}
